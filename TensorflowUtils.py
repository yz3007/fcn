...
def get_variable(weights, name):
    init = tf.constant_initializer(weights, dtype=tf.float32)  # weights: initilize the variable
    var = tf.get_variable(name=name, initializer=init, shape=weights.shape)

    return var


# how to init variable well ???????????????????????????????????
        ...
        if in_chans == out_chan2:
            ...
            with tf.variable_scope('branch1'):
                if up_stride:
                    ...
                    b1 = conv(x, out_chans=out_chan2, shape=1, strides=frist_stride,
                              name='res%s_branch1' % name)
        with tf.variable_scope('branch2a'):
            if up_stride:
                b2 = conv_tranpose(x, out_chans=out_chan1, shape=1, strides=first_stride,
                                   name='res%s_branch2a' % name)
            else:
                b2 = conv(x, out_chans=out_chan1, shape=1, strides=first_stride,
                          name='res%s_branch2a' % name)
