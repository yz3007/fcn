...
def color_transform(label_image, num):
    """
    convert label_image to fake color image
    :param label_image: (h,w) the value of each pixel represents each diff classes
    :param num: number of classes to be predicted including background color
    :return: fake color image
    """

    cmap = labelcolormap(num)

    im_rgb = cv2.cvtColor(label_image, cv2.COLOR_GRAY2RGB)

    imsz = label_image.shape
    im = np.array(label_image)
...
def draw_edge(edge, image):
    ...
    :param edge: edge generated by sobel() (h, w)
    :param image: original image(RGB)
    :return:
    """
    if edge.shape[0] != image.shape[0] or edge.shape[1] != image.shape[1]:
        raise RuntimeError('edge shape should be equal to image shape in first 2 dims')
    # print edge
    # print edge.shape
    # print image
    # print image.shape
    image[edge != 0] = [255, 0, 0]
    return image
...
def sobel(img):
    """
    detect figure edges
    :param img: is supposed as greyscale
    :return dst: sobel edge
    """
